  ...
  export default {
    ...
    methods: {
      ...
      async upload(e) {
        ...
        let res = await this.$axios.$post('/upload', data, conf).catch(e => {
          this.form = {
            file: null,
            mode: null,
            time: 120
        ...
        let analyze = await this.$axios.$post('/analyze', {
          path: res.path,
          mode: this.form.mode,
        ...
        this.$router.push({'name': 'results-resultid', params: { resultid: analyze.UUID }});
      }
    }
  }
</script>
